...
"""
BiDAF code was gratefully adapted for the task of binary QA from this great github repository: 
...
class LSTM(nn.Module):
    """
    An auxilary class of the nn.LSTM with the forward function.
    Useful for code compactness, as LSTM will be used multiple times.
    """
...
class BiDAF(nn.Module):
    def __init__(self, args, char_vocab_size, pretrained_matrix):
        super(BiDAF, self).__init__()
        self.args = args

        # 1. Character Embedding Layer
        self.char_emb = nn.Embedding(char_vocab_size, args.char_dim, padding_idx=1)
        ...
        self.word_emb = nn.Embedding.from_pretrained(pretrained_matrix, freeze=True)
        ...
        # 6. Output Layer
        self.outp_weight_g = nn.Sequential(nn.Linear(args.hidden_size * 8, 2), nn.Dropout(p=args.dropout))
        self.outp_weight_m = nn.Sequential(nn.Linear(args.hidden_size * 2, 2), nn.Dropout(p=args.dropout))

        # self.p1_weight_g = nn.Linear(args.hidden_size * 8, 2, dropout=args.dropout)
        # self.p1_weight_m = nn.Linear(args.hidden_size * 2, 2, dropout=args.dropout)
    ...
    def forward(self, batch):
        ...
        def output_layer(g, m):
            ...
            """
            # (batch, c_len)
            preds = (self.outp_weight_g(g) + self.outp_weight_m(m)).squeeze()

            return preds

        c_word = batch["word_contexts"]
        q_word = batch["word_questions"]
        c_char = batch["char_contexts"]
        q_char = batch["char_questions"]

        # 1. Character Embedding Layer
        c_char = char_emb_layer(c_char)
        q_char = char_emb_layer(q_char)
        # 2. Word Embedding Layer
        c_word = self.word_emb(c_word[0])
        q_word = self.word_emb(q_word[0])
        c_lens = c_word[1]
        q_lens = q_word[1]
